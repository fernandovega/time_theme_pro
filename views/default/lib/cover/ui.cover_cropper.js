...
elgg.provide('elgg.coverCropper');
 ...
 */
elgg.coverCropper.init = function() {
	var params = {
		selectionOpacity: 0,
		aspectRatio: '3:1',
		onSelectEnd: elgg.coverCropper.selectChange,
	};
	...
	if ($('input[name=x2]').val()) {
		...
		elgg.coverCropper.preview($('#user-cover-cropper'), selection);
	}
};
 ...
 * @return void
 */
elgg.coverCropper.preview = function(img, selection) {
 ...
 * @return void
 */
elgg.coverCropper.selectChange = function(img, selection) {
...
elgg.register_hook_handler('init', 'system', elgg.coverCropper.init);